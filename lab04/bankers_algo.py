...
# ทรัพยากรระบบ: [GPU_Type_A, GPU_Type_B, High_Speed_RAM(TB)]
total_resources = np.array([10, 5, 7])

# ความต้องการสูงสุดของแต่ละ ML Job (Job 0, Job 1, Job 2)
max_need = np.array([
    [7, 5, 3], 
    [3, 2, 2], 
    [9, 0, 2]
...
def is_safe_state(available, max_need, allocated):
    ...
    # คำนวณความต้องการที่เหลืออยู่: Need = Max - Allocated
    ...
    while len(safe_sequence) < num_jobs:
        ...
        for i in range(num_jobs):
            if not finish[i] and all(need[i] <= work):
                # Job i สามารถทำงานจนเสร็จได้
                work += allocated[i] # OS ดึงทรัพยากรคืน
                finish[i] = True
                safe_sequence.append(f"Job_{i}")
                allocated_in_this_round = True
                
        if not allocated_in_this_round:
            return False, [] # ระบบอยู่ในสถานะไม่ปลอดภัย (เกิด Deadlock แน่นอน)
...
def main():
    ...
    # คำนวณทรัพยากรที่ว่างอยู่ในปัจจุบัน
